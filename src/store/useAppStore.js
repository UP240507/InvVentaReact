...
import { siguienteIdUnico, SERIE_AUDITORIA } from '../lib/IdVenta';
// ── HELPER UTC ────────────────────────────────────────────────────────────────
...
export const useAppStore = create((set, get) => ({
  ...
  //
  // ── LA AUDITORÍA PASA POR LA COLA, COMO TODO LO DEMÁS ────────────────────
  // Antes hacía `supabase.insert` directo bajo un `if (navigator.onLine)`, y de
  // ahí salían dos agujeros, los dos callados:
  //
  //   1. **Sin red no se intentaba siquiera.** La línea se guardaba en Dexie y
  //      nadie recordaba subirla nunca: no había cola, ni reintento, ni copia
  //      en el hub. Se perdía con el aparato.
  //   2. **Con red pero sin llegar a Supabase** —wifi sin salida, que es el
  //      caso corriente en un local— el insert fallaba y el error moría en un
  //      `console.error`. En un build de release no hay consola que abrir.
  //
  // Medido en AZUL el 17-ago: la venta `AZULHN-V-000005` está en los libros a
  // las 20:09 y la auditoría se corta a las 19:40. **Un cobro sin rastro, en la
  // pantalla que se llama «Registro inmutable».** Y el hueco cae justo donde un
  // auditor miraría con más atención: un dispositivo que desapareció con dinero
  // dentro. La discrepancia no distingue un teléfono sin batería de alguien
  // borrando lo suyo.
  //
  // Con `enqueueAction` hereda lo que ya funciona: persistencia en Dexie,
  // reintentos con backoff, y —desde hoy— copia en el hub, porque `auditoria`
  // entra en `TABLAS_RESPALDADAS`.
  //
  // ── Y POR ESO EL `id` DEJA DE SER `Date.now()` ──────────────────────────
  // Para respaldarse hace falta una clave de deduplicado, y `Date.now()` la
  // comparten todos los aparatos: dos teléfonos registrando en el mismo
  // milisegundo darían la misma. Es el mismo fallo que `IdVenta.js` arregló
  // para las ventas, y la serie ya estaba reservada sin usar (`SERIE_AUDITORIA`).
  // La columna en Postgres es `GENERATED BY DEFAULT`, así que acepta el nuestro.
  registrarAuditoria: async (logInfo) => {
    const fila = {
      id: logInfo.id || siguienteIdUnico({ serie: SERIE_AUDITORIA }),
    ...
    set((state) => ({ auditoria: [fila, ...(state.auditoria || [])] }));

    try {
      await localDB.auditoria.put(fila);
      // Sin `await`: registrar una acción no puede hacer esperar a quien la
      // hizo. La cola ya garantiza que no se pierde.
      void useSyncStore.getState().enqueueAction('auditoria', 'insert', fila);
    } catch (err) {
